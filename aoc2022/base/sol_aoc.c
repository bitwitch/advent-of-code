...
void setupgif(uint32_t delay, int motionblurframes, const char* fn)
{
    ...
    framedata = malloc(512 * 512 * frames * sizeof(unsigned int));
	frameidx = 0;
    globalDelay = delay;
...
void nextframe(int ofs)
{
	...
	GifWriteFrame(&gifwriter, (const unsigned char*)frame, 512, 512, globalDelay, bitDepth, dither);
}
...
void drawstringf(int aX, int aY, int n, int aColor, const char* fmt, ...)
{
	...
	va_end(args);
	drawstringn(buffer, aX, aY, n, aColor);
}

void drawcharn(int aChar, int aX, int aY, int n, int aColor)
{
	int i, j;
	for (i = 0; i < 12*n; i++)
	{
