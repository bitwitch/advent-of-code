...
void simulate(Monkey *monkeys, U64 turn_count, U64 worry_divisor) {
    Monkey *m;
    ...
    for (turns=0; turns<turn_count; ++turns) {
        for (i=0; i<(U64)arrlen(monkeys); ++i) {
            ...
            for (j=0; j<(U64)arrlen(m->items); ++j) {
                ...
                printf("Monkey %llu: item=%llu new=%llu target=%llu\n", i, item, new, target);
            }
            // clear the array
            arrsetlen(m->items, 0);
        }
    ...
    for (i=0; i<(U64)arrlen(monkeys); ++i) {
        ...
        printf("Monkey %llu inspected %llu items\n", i, monkeys[i].items_inspected);
    }
...
int main(int argc, char **argv) {
    ...
    Monkey *monkeys_one = parse_monkeys(file_data, file_size);
    int len = arrlen(monkeys_one);
    Monkey *monkeys_two = NULL;
    arrsetcap(monkeys_two, len);
    arrsetlen(monkeys_two, len);
    memcpy(monkeys_two, monkeys_one, len * sizeof(Monkey));

    /*simulate(monkeys_one, 20, 3);*/
    simulate(monkeys_two, 20, 1);
